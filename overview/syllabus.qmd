...
from, but "publishing" this type of information isn't rewarded so it is
rarely done. Likewise, the code used in analyses is rarely shared, even
though it would improve the reproducibility of the research results and
also allow others to learn from it. Ultimately, all of this leads to
...
-   [Extend your analysis to run many models](/sessions/many-models.qmd)
-   [Visualising the results of many
    models](/sessions/visualise-models.qmd)
...
-   Run multiple statistical analyses efficiently using `{purrr}` and
...
`{styler}`, `{targets}`, as well as more advanced uses of combining
`{purrr}` and `{dplyr}`.
