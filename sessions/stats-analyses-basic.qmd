...
We want to use logistic regression. So, open the `doc/lesson.qmd` file
...
Next, in the `doc/lesson.qmd` file, on the bottom of the document create
...
chunk at the bottom of the `doc/lesson.qmd` file, like listed below:
...
We will eventually delete these exercise text in the R Markdown file,
but for now, commit the changes to the Git history.
...
`mutate()` with `snakecase::to_snake_case()`. In the `doc/lesson.qmd`
...
`r run_roxygen_comments`, style using {{< var keybind.styler >}},
`source()` the modified `R/functions.R` file, and add the new function
above the `pivot_wider()` code in the `doc/lessons.Rmd` file.
...
7.  Replace the code in the `doc/lesson.qmd` file to make use of the new
    functions.
...
Replacing the code in the `doc/lesson.qmd` file to use the function.
