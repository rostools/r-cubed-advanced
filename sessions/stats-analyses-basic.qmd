...
`mutate()` with `snakecase::to_snake_case()`. In the `doc/lesson.Rmd`
...
-   `estimate`: This column is the "coefficient" linked to the term in
    ...
    $$ class = Intercept + (metabolite\_estimate \times metabolite\_value) + \\ (gender\_estimate \times gender\_value) + ...$$
