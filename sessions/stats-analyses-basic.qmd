...
For this session, we are going to put a pause on targets and learn how
we can generally set up and run statistical models.
...
3.  Identify data transformation techniques and evaluate which are good
    options given the data. Use functions in the `{recipes}` package to
    apply these transformations to your data.
...
This means that what we show is *for demonstration purposes only*, they
could be entirely wrong in how to do them correctly if an expert were to
review them.
...
(independent variable) ( x ), how does that affect the outcome
...
-   "What is the relationship between a metabolite and T1D compared to
    controls, adjusting for age and gender?"
...
corresponding mathematical object is a function:
...
### Workflow for Statistical Models
...
`{tidymodels}` make it easy to create and use models in a flexible way,
...
`{tidymodels}` are designed to clearly separate the different parts of
