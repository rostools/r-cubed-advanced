...
for `Cholesterol`. This is likely a data entry or smalling mistake
...
columns except for `value`. Then we can use the `.groups = "keep"`
argument in `summarise()` to keep the grouping variables since we want
them kept. We have to use `pick()` in `group_by()` in order to select
...
Since this issue affects our other outputs that we made before, like the
table and plot we made in the last sessions. So we have to apply this
fix right after reading in the data, so all other targets use the
corrected data.
...
have the same units and same distribution shape.
...
the [odds ratio](https://en.wikipedia.org/wiki/Odds_ratio). Let's also
add the confidence intervals by setting `conf.int = TRUE`:
...
3.  Add Roxygen documentation to the function with
    {{< var keybind.roxygen >}}.

4.  Test the function with the code above to make sure it works as
    expected. If it does, then cut and paste the function into the
    `R/functions.R` file.

5.  Run styler with {{< var keybind.styler >}}.

6.  Commit the changes to the Git history with {{< var keybind.git >}}
    and push to GitHub.
...
## :technologist: Exercise: Add the results function as a target

**Time: \~8 minutes.**

Taking the `create_model_results()` function from above, next step is to
add it to the pipeline in the `_targets.R` file! Use the below scaffold
as a guide.
...
-   Before coding, design and sketch out how the data will flow through
    different actions that will eventually lead to the output you want.
...
-   As much as possible, functions should output a data frame because
    they are easier to work with and work very well with `{tidyverse}`
