...
graph TB
    ...
    plot_est -- "tar_read()" --> rmd[R Markdown]
...
We want to use logistic regression. So, open the `doc/learning.qmd` file
...
Next, in the `doc/learning.qmd` file, on the bottom of the document
create a new header and code chunk:
...
#| column: page-inset-right
metabolite_values_to_snake_case <- function(data) {
  data %>%
    dplyr::mutate(metabolite = snakecase::to_snake_case(metabolite))
}
lipidomics %>%
  metabolite_values_to_snake_case()
```
