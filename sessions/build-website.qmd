...
of R Markdown that is familiar to many R users.
...
`doc/lesson.qmd` file from being rendered (since we will use this file
...
#| purl: true
#| eval: false
fs::file_move("doc/lesson.qmd", "doc/_lesson.qmd")
```
...
git_ci(c("doc/lesson.qmd", "doc/_lesson.qmd"), "Rename file lesson file.")
...
to the whole project. Cut and paste that metadata from `doc/lesson.qmd`
...
-   Use Quarto code chunk options, along with `{ggplot2}` theme
    functions, to make figures more visually appealing on their own and
    in the document.
