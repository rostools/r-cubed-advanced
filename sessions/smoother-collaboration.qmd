...
-   Disadvantages:
    ...
    -   When you restore a library from a renv.lock file that utilize
        old package versions, it can sometimes be cumersome to
        re-install them.
...
scanning the `DESCRIPTION` file. While we allready selected to use
explicit declaration when initilizing `{renv}`, this option is specific
for our R instance. In order for `{renv}` to always work explicit, we
need to set an project option for it. This option needs to be added to
