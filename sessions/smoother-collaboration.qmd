...
1.  Open your `doc/lesson.qmd` file, create a new code chunk
...
3.  Go back to `doc/lesson.qmd` file in RStudio and run `lintr::lint()`
...
4.  Run `styler::style_file()` using the Command Palette (`Ctrl-Shift-P`
    then type "style file") while inside the `doc/lesson.qmd` file. What
...
`doc/lesson.qmd` file and go to the top of the file. Right below the
...
editor_options <-
  ...
  markdown:
    wrap: 72
    canonical: true"
readLines("doc/lesson.qmd") |>
  append(editor_options, after = 1) |>
  writeLines("doc/lesson.qmd")
git_ci("doc/lesson.qmd", "Add editor options to Rmd file.")
```
...
Let's test it out. While in the `doc/lesson.qmd` file, go to the bottom
