...
1.  Identify actions to streamline collaboration on a data analysis
    project and create projects that apply many of these actions using
    R.  
...
## Exercise: How do you collaborate or contribute to your own or others' projects?
...
to track the project's package dependencies, and, ideally, have a simple
...
the common approach of using `setwd()`. The R Project is made of metal,
...
in casts and band-aids, looking much more fragile. [Artwork by
...
it. As a result, you will have to re-run the script a few times if you
...
-   Advantages:
    ...
    -   Because it is widely used, there are many processes already
        ...
        instance, when creating R package, you need to track package
...
-   Disadvantages:
    ...
    -   Your project might still rely on a package that is installed on
        *your* computer which influences your project. It could be that
        ...
        In other terms, you are still responsible for adding the project
...
## Exercise: Add packages from the data processing script to your project {#sec-data-processing-dependency-exercise}
...
actually build dependencies, before we use them directly in our project
to process the data.

The way you add packages to the `DESCRIPTION` file is sligthly different
...
let's tweak our `.Rprofile` to make our lives a bit easier. We'' add a
...
collaborating on a project, you can run `pak::pak()` to installs all
types of dependencies, both workflow and build. Neat!
:::
...
{{< var keybind.styler >}} to style the file. Notice that there isn't
any changes since the file is already tidy.
...
might sometimes have to manually run the reformatting (with the Command
Palette {{< var keybind.palette >}} then "style active file").
...
to writing R Markdown files (which is great if you are more comfortable
...
## Exercise: Update the README file while using the canonical Markdown mode
