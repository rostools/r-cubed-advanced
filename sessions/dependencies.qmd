# Dependency management for smoother collaboration {#sec-dependencies}
...
perhaps want to---*directly collaborate* much more with others.

::: callout-tip
Here, "directly collaborate" means contributing to a shared project
directly (like editing the same files), rather than discussion- or
planning-based collaborations (and definitely not the
"emailing-files-around" type of collaboration).
:::
...
## :book: Reading task: Project-level package dependency management

::: {.callout-note collapse="true"}
## :teacher: Teacher note

Briefly highlight the meaning of "project-oriented workflow", the
informal way of tracking dependencies, and why it's not optimal.
:::

**Time: \~8 minutes.**

::: callout-note
This first session is more conceptual and is heavier on the reading and
explanation, but is important for the next sessions.
:::
...
software everyone will need in order to use or contribute to your
project. This starts out at the highest level: Are you using R or some
other software for the analysis? Since this is an R workshop, we're
assuming the software will be R! 😜
...
Projects](https://r-cubed-intro.rostools.org/sessions/setup-project/),
...
the use of the `{here}` package. So, we're already following this
approach, which will make it easier to track the package dependencies
for our project.
...
Let's start with the `AdvancedR3` project, created in
@sec-create-project, that includes the `lipidomics` dataset. We have
code in the `data-raw/nmr-omics.R` file already that uses some packages.
Let's assume that your project will be more complex than this and that
you will eventually need some collaborators to contribute to your
project who are experts in, for instance, metabolomics data processing
and in statistical analysis of high-dimensional data. You know you will
end up needing to use other packages. You also know that you all need
some way of tracking which packages are used so that when others join
and contribute to the project, they can as seamlessly as possible
install or update the packages your data analysis project needs. There
are a few ways of "tracking" package dependencies.
...
## :book: Reading task: Formal dependency management

::: {.callout-note collapse="true"}
## :teacher: Teacher note

Reinforce that we track dependencies via the `DESCRIPTION` file and
`usethis::use_package()`, as well as the easy of installing and updating
with `pak::pak()`.
:::
...
the `DESCRIPTION` file and `usethis::use_package()`. We covered this
style of dependency management in the [intermediate
workshop](https://r-cubed-intermediate.rostools.org/sessions/robustness/).
...
-   **Advantages**:
    ...
    -   Relatively easy to conceptually understand, since you can
        directly view the packages used in the project by opening the
    ...
    -   Because it is widely used, there are many processes already
        ...
        dependencies using this approach.
    ...
    -   Adding packages that your project will depend on is as easy as
        writing `usethis::use_package("packagename")` in the Console.
...
which we covered in previous versions of this workshop. It is a great
...
complex and difficult to understand. It is also not really necessary for
the majority of projects.
:::

As we work on the project during the workshop and realize we need to use
a specific package, we will continue using `usethis::use_package()` to
...
Before continuing with the exercise, be sure to add and commit all the
...
the `data-raw/nmr-omics.R` script. Open the `data-raw/nmr-omics.R` file
...
3.  In the Console, run `usethis::use_package("packagename")` for each
    package you find in `data-raw/nmr-omics.R` (from item 1. above).
...
1.  Packages that *directly* contribute to data wrangling, analysis,
    ...
    use them directly in scripts for processing data.
2.  Packages that *assist* you in doing your work but aren't directly
    ...
    considered a **workflow** dependency because you usually only use it
    in the Console and don't need to use it in a script.

A useful guideline to help you determine whether a package is a build or
a workflow dependency is:
...
(in `data-raw/nmr-omics.R`) to process the data.
...
What this code does is that when you start an interactive R session
(like when you open RStudio), it will automatically load the `{usethis}`
package without you having to type `library(usethis)`. It means you
don't need to always use `usethis::` before any of its functions when
you use them. This is template code, so we don't really need to
understand the details of what it is doing, but briefly, the
`interactive()` function checks if you are running R interactively (like
in RStudio with the Console open). The `suppressMessages()` function
prevents R from printing messages when loading the package and the
`require()` function loads the package (if it is installed).
...
You've used `{styler}` during the pre-workshop tasks and we'll use it
more throughout the workshop. Which dependency do you think this is?
It's a **workflow** dependency, as we will only use it in the Console or
Command Palette. Let's add it to the project `DESCRIPTION` as a workflow
dependency:
...
5.  Check your GitHub to make sure the project repository has been
    uploaded to it. You can use `{usethis}` for help:
...
contributions. We've already set up and used `{styler}` during the
pre-workshop tasks to automatically format R code.
...
Markdown mode in the "source" (default) mode.

Let's test it out. While in the `docs/learning.qmd` file, go to the
...
`docs/learning.qmd` file through the sessions, we'll get lots of
experience using it.
...
Then delete the `TODO.md`, followed by committing the deletion in the
...
for smoother collaboration with formal dependency management:
...
-   Install the dependencies necessary for your project with
    `pak::pak()`.
-   Differentiate between build and workflow dependencies and how to add
    them to the `DESCRIPTION` file.
