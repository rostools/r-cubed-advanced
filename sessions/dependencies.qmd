...
perhaps want to---*directly collaborate*[^dependencies-1] much more with
others.

[^dependencies-1]: Here, "directly collaborate" means contributing to a
    shared project directly (like editing the same files), rather than
    discussion- or planning-based collaborations (and definitely not the
...
It's a **workflow** dependency, as we will mostly likely only use it in
the Console or Command Palette. Let's add it to the project
`DESCRIPTION` as a workflow dependency:
