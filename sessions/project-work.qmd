...
team will take what you learned in the course and apply it to create a
...
1.  Get together with your team. We will already have created a
    ...
    course. Clone your team repository to your computer using
...
9.  As a team, decide which variables you want to analyse and split the
    ...
    details.
10. Using the workflow we've covered in this course, collaboratively
...
-   **Onset of Labor** dataset:
    -   In this dataset, there are few response variables (`y`) and
        *many* predictor variables (`x`).
    -   We suggest that each person on a team of two select one of the
        two response (`y`) variables for the model, which are `EGA`
    ...
    -   Since there are many predictor variables (`x`), we suggest each
        ...
        include any variable *except* the above response variables as
        well as `ID`, `SampleID`, and `Timepoint`.
-   **Bearded Dragon** dataset:
    -   We suggest each person select *one* response (`y`) variable for
...
-   **General analysis tips**:
    -   Stick to simple model formulas by using only one response and
    ...
    -   We *strongly* suggest you do not include more than one predictor
        ...
        and coding more complicated than is necessary for this course.
