...
where you have to re-run some analyses and have forgotten the order that code should run or which parts you need
to re-run in order to update the results. Things can get confusing quickly, even
in relatively simple projects. This becomes even more challenging when
you return to a project after a month or two and have forgotten the
state of the analysis and the project as a whole. Add in collaborators,
and things can get even more complex.
...
re-run code and update your results? Assume it's a change in the code somewhere
...
2.  For 8 minutes, share and discuss your thoughts in with your neighbour. How do
...
So why are these computational data pipelines rarely found in most of the
research "pipelines" described in papers? We can break the reason why not
into specific points that together explain why not:
...
reproducible data analysis. But based on the above points,
researchers (on average and as a group) don't have the technical
expertise and knowledge to actually implement data analysis pipelines.
...
use very
rarely being allowed to get (or use) funds to hire people who *are* trained
and skilled in programmatic designing, thinking, and coding. Otherwise workshops
like this wouldn't need to exist :woman_shrugging:
...
and *data analysis for the final paper*. In exploratory data analysis, there
...
and pipeline. But for the final paper and the included results, you
...
Here, there are three distinct items ("products") that you (might) want to create
from your data. Each "flow" from data to the desired product has specific
actions that need to happen. Now that you've seen how it is conceptually
drawn out, there are some initial tasks you can work towards in
your pipeline. In this session, we will build up the actions
...
This package allows you to explicitly set "targets" (objects like the table or figure)
you want to create as part of the pipeline. `{targets}` will then track all your targets
for you and tracks which output a target is input to another target as well as which
target need to be updated when you make changes to your pipeline.
...
using `{targets}`! There's a handy helper function that does this for us:
...
running on a Linux server) that we won't cover in this workshop. You can safely
delete them.
:::
...
For this reading task, you'll take a look at what `_targets.R`
...
As a result, you might end up with a "main" R file that has code that looks
like:
...
list(
  ...
  tar_target(
    name = raw_data,
    command = load_raw_data(raw_data_path)
  ),
  tar_target(
    name = processed_data,
...
`{targets}` pipeline is structured using a function-oriented workflow, the
pipeline can also be visually represented as a graph, which we show in
@fig-targets-as-flow.

```{mermaid}
%%| label: fig-targets-as-flow
%%| fig-cap: Representing the targets pipeline structure as a flow diagram, where each "node" is an R object and each arrow is the function that converts the object to another object.
%%| echo: false
graph TB
    file -->|"load_raw_data()"| raw_data
...
{{< var keybind.targets-make >}} rather than the function in the
Console.

After running it, you should in one of RStudio's panes something that
looks like:
...
Inside this folder it keeps all of the output from running the code in
`_targets.R`. It comes with its own `.gitignore` file so that you don't
track all the files inside, as they don't need to be tracked. Only the
`_targets/meta/meta` is needed to be tracked in Git.
...
We can also use the keybind {{< var keybind.targets-vis >}}, which is what
we will use for the rest of the workflow rather than the Console command.
...
If you want to
check which targets are outdated, we use:
...
Or by using the keybind {{< var keybind.targets-outdated >}}.
...
the `file` target as input. In the `_targets.R` file at the
`list()` section at the bottom, add another `tar_target()` that
looks like:
...
We use `show_col_types = FALSE` to suppress the message about which
columns are being read into R. Now, let's run `{targets}` again using
{{< var keybind.targets-make >}}. After it runs, check the visualisation
of it with {{< var keybind.targets-vis >}}. You can now see that there
are two targets, with an arrow from `file` to `lipidomics`.

::: {.callout-note collapse="true"}
## :teacher: Teacher note

Walkthrough this again, this time showing the new target added and the
arrow between them.
:::

How can we access that new `lipidomics` data that `{targets}` created? For that, we use the
function `targets::tar_read()`. `{targets}` stores the output of each
target in its own internal storage system, which is kept in the
`_targets/` folder. So when we use `targets::tar_read()`, we access any
stored target items using syntax like we would with `{dplyr}`, without
quotes.
...
We're now ready to start building our first item/product! But first, since we
finished writing some code, let's style `_targets.R` using
...
-   Visualize target items in your pipeline with
    ...
    `targets::tar_outdated()`.
-   Use `targets::tar_read()` to
    access saved pipeline outputs.
