...
4.  Run `use_package()` for the two packages you used inside your function
    that are not already in the `DESCRIPTION` file.
...
Show this diagram again on the screen to remind the learners of the overall
...
We've learned that we can create data frames as outputs from our pipeline,
but the nice thing with `{targets}` is that we can create other types of
objects too, like figures!

Let's write code to create our first plot product. Since we're using `{ggplot2}` to
make these figures, we'll add it to our `DESCRIPTION` file.
...
Slowly build up this plot code, running it each time you add a new line/layer
and explaining what it does and why we're doing it.
:::
...
nice figures, it's mainly about how to build reproducible pipelines.
So for now, run `{styler}` with {{< var keybind.styler >}},
then run the pipeline with {{< var keybind.targets-make >}} to check that
everything builds correctly. Open the updated HTML file to check how it looks.
Then, we'll add and commit the changes to the Git history
with {{< var keybind.git >}} and push the changes up to GitHub.
...
`create_table_descriptive_stats()` function in the exercise above, complete these
tasks, using the scaffolding code below to help you get started:
...
1.  Wrap the plot code inside `docs/learning.qmd` with
    `function() {...}` and name the new function `create_plot_distributions`.

2.  Replace `lipidomics` with `data` and put `data` as an argument
    inside the brackets of `function()` (already done in the scaffolding code).
...
7.  Source the `R/functions.R` file with {{< var keybind.source >}} and
    then test the code by running `create_plot_distributions(lipidomics)` in
