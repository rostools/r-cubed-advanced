...
for rendering the Quarto file called: `tar_quarto()`. Like `tar_target()`, this
takes a `name` argument. But unlike `command` for `tar_target()`, it takes a
`path` argument, which is the file path to the Quarto file. Let's add it:
