...
If you recall from @fig-pipeline-schematic, we had three items (that we call products) that we
...
Let's add these products into the design of our `{targets}` pipeline that we made in @fig-our-pipeline-design.
We'll indicate that our first two targets have been made in the graph,
which now looks like what is shown in
@fig-first-two-done-pipeline.
...
`plot_distributions` targets, then include them in the `report`.
In order for us to slowly build this up, we will start at the end first:
rendering our Quarto document to HTML in our `{targets}` pipeline.

Open up the `docs/learning.qmd` file
and create a new code chunk at the bottom of the file.
Name the code chunk `setup` (using the chunk label `{r setup}`).

We'll be using some `{targets}` functions in this document, so let's add
`library(targets)` to the `setup` code chunk.
Since we'll eventually be adding code as functions into the `R/functions.R`
file, we'll `source()` it right now in the `setup` code chunk and using `here::here()` to
give the correct file path to the `R/functions.R` file.
Lastly, in order use the `lipidomics` data that we added to the `_targets.R` pipeline
in the last session (and that is also stored in the `_targets/` target "store"),
we use `tar_read()` to read it in and assign it to the  `lipidomics` object.
...
We will likely get an error saying something about not finding the `{targets}`
"store". This is because when we use a `{targets}` function like `tar_read()`,
it will look for a folder called `_targets/` in the same folder that the
R session is running in. When we render a Quarto document, the default location
for the R session is the folder where the Quarto file is located, which is `docs/`.
We don't have a folder called `docs/_targets/`. Instead, it is in the root of the project.
So we have to tell `{targets}` where to find the store, which we can do by using
`targets::tar_config_set()`. This function allows us to set various
configuration options for `{targets}`. One of these options is `store`, which
we can use to tell `{targets}` where the `_targets/` store is located relative to
the Quarto file.

So in the `setup` code chunk, we'll add the `tar_config_set()` line right
after `library(targets)`. Since the Quarto file is in the `docs/` folder,
we can use the relative path `"../_targets"` to point to the store.
...
Let's try to render the file now with {{< var keybind.render >}}. It should
work now without any errors! :tada:

Great! Now let's include the Quarto file as a target in our pipeline.
Open up the `_targets.R` file. But, how do we add a Quarto file as a target?
If we try to look through the `{targets}` documentation, we won't find anything
specific for Quarto files. However, there is a helper package called
`{tarchetypes}` that contains functions that make it easier to add
specific types of files as targets, including Quarto files. You can see
at the top of the `_targets.R` file that there is a commented out
line for loading the `{tarchetypes}` package. So let's uncomment that
line so that it looks like this now:
...
We don't have the `{tarchetypes}` package installed yet, so let's install it
by going to the Console and using `use_package("tarchetypes")`.
...
Now, let's go back to the `_targets.R` file and go to the bottom of the file
with the `list()` of `tar_target()` items. We will now add the new item
for rendering the Quarto file that is called `tar_quarto()`. Like `tar_target()`, this
takes a `name` argument. But unlike having a `command` argument like in `tar_target()`, `tar_quarto()` takes a
`path` argument where you write the file path to the Quarto file. Let's add it:
...
Since we've made some changes, let's first style the file using {{< var keybind.styler >}}.
Now let's run
`targets::tar_make()` with {{< var keybind.targets-make >}} to build the Quarto
document. However, we will probably get an error because we haven't installed
the `{quarto}` R package yet.

<!-- TODO: Confirm error message -->

So let's install the `{quarto}` R package to help `{targets}` connect to the Quarto file:
...
Now, let's build the pipeline with {{< var keybind.targets-make >}} again.
It should work! :tada: Let's also check the visualization of the pipeline
with {{< var keybind.targets-visual >}}. You should now see the `quarto_doc`
target at the end of the pipeline.

When we run the pipeline with the Quarto target in it, we'll see
that a new file is created in the `docs/` folder.
When we use `targets::tar_config_set(store = ...)`, it creates a new
file called `_targets.yaml` that contains details
that tell `{targets}` where to find the store. Since the path listed
in this `docs/_targets.yaml` file is a relative path to the store, we can
add it to our Git history so that our collaborators (or future you) also use it.

So let's commit the changes we've made to our project into the Git history with
{{< var keybind.git >}}.
...
We'll add `library(tidyverse)` to the `setup` code chunk, so that we can
use the functions from `{tidyverse}`.

For
this, we will call our "final" function that we will include in the `{targets}` pipeline
`create_table_descriptive_stats()`. This function will take the `lipidomics` data as
input (the first argument) and return the `table_descriptive_stats` as output.

To create this `table_descriptive_stats` object, we'll use `{tidyverse}`, specifically `{dplyr}`, to calculate the
descriptive statistics from the `lipidomics` data. We don't yet have it added
to our dependencies, so let's add it! Because `{tidyverse}` is a "meta"-package (special
...
We can represent this visually, where each block is an R data frame, it
would like:
