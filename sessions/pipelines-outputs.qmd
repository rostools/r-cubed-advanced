...
For this, we will call our "final" function that we will include in the
`{targets}` pipeline `create_table_descriptive_stats()`. This function
will take the `lipidomics` data as input (the first argument) and return
the `table_descriptive_stats` as output.

To create this `table_descriptive_stats` object, we'll use
`{tidyverse}`, specifically `{dplyr}`, to calculate the descriptive
statistics from the `lipidomics` data. We don't yet have it added to our
