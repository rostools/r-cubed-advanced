...
given data in a '.rda' format. Instead `{targets}` want a "raw" datafile
that itself can store as '.rda'. To account for this we need to alter our data
wrangling script. As the data wrangling in not part of our actual workflow and 
we don't want `{renv}` to track the packages used we will momentarily deactivate 
`{renv}` using `renv::deactivate()`. From here we will open up
the the `nmr-omics.r` script and replace:
...
``` r
readr::write_csv(lipidomics, "data/lipidomics.csv")
``` 
We can now source the script (to generate a new data file), reactivate our `{renv}` 
environment using `renv::activate()`, and update the `_targets.R` file. 
...
list(
    ...
    tar_target(
        name = lipidomics,
        command = read_csv(file, show_col_types = FALSE)),
    tar_target(
        name = df_stats_by_metabolite,
        command = descriptive_stats(lipidomics)
    )
...
As we are now using `{readr}` we should add this to the DESCRIPTION file.

Now, let's try running `{targets}` again! You can either use the
Command Palette (`Ctrl-Shift-P`, then type "run targets") or run this
code in the Console:
