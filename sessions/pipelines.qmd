...
This will write a `_targets.R` script file that we can use to define our
analysis pipeline. Before we do that though, let's **commit the file to
...
running on a Linux server) that we won’t cover in the course.
:::
...
continuous lipid variables, and a report (Quarto). Since we'll use
...
project library. Because `{tidyverse}` is a "meta"-package (special type
of package that makes it easy to install and load other packages) we
need to add it to the `"depends"` section of the `DESCRIPTION` file.
...
make it more readable, to round the numbers to one digit. To do this,
will utilize the Split-apply-combine methodology and the `across()`
function, we covered in the
...
3.  Round the calculated mean and sd to 1 digit using `round()`.
...
1.  Wrap the code with `function() {...}` and name the new function
    `descriptive_stats`. Here is some scaffolding to help you get
    started:
...
actual data file we need to create a pipeline target of the data file.
We can accomplish this by using the argument `format = "file"` inside
