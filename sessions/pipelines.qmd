...
We can now source the script (to generate a new data file) and update
the `_targets.R` file. Because of the way `{targets}` works, we need to
first create a pipeline target of *only* the path to the data file, and
then we can load it with `{readr}`.
