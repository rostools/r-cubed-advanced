...
descriptive statistics, a plot of the continuous lipid variables, and a
report (R Markdown). Since we'll use `{tidyverse}`, specifically
`{dplyr}`, to calculate the summary statistics, we need to add it to our
dependencies and install it in the project library. `{tidyverse}` is a
special "meta"-package so we need to add it to the `"depends"` section
...
Git history with {{< var keybind.git >}}. Open up the `doc/learning.qmd`
...
{{< var keybind.targets_make >}} or run this code in the Console:
...
While this `{targets}` pipeline work, we would not be able to
automatically rerun out pipeline if our underlying data changes. To
track the actual data file we need to create a pipeline target of data
file. We can acomplish this using the argument `format = "file"` inside
`tar_target()` before loading the data using `{readr}`.
...
This function will find all uses of packages as dependencies in a
project, so it can also list duplicate packages. To show only the unique
packages used, we'll use `unique()` and `$Package` (the variable name),
as well as stopping the function from outputting messages with
`quiet = TRUE`:
...
This code we can now place in the `packages` argument of
...
revise_by_text(
  path = "_targets.R",
  original = "packages = .*$",
  replacement = "packages = unique(renv::dependencies(quiet = TRUE)$Package),"
)
```

Try running `{targets}` again with either `targets::tar_make()` or
{{< var keybind.targets_make >}}. It should run through! We also see
that a new folder has been created called `_targets/`. Since we don't
want to track this in Git, we should ignore it with:
...
#| echo: false
#| purl: true
git_ci(c(".gitignore", "_targets.R", "DESCRIPTION", "renv.lock"), 
       "Tell targets about packages, ignore _targets/")
```
...
write this code, let's add it to our `DESCRIPTION` file and to our
...
{{< var keybind.targets_vis >}} or running `targets::tar_outdated()`
with {{< var keybind.targets_outdated >}}. You should see that the new
item is "outdated". Then run `targets::tar_make()` using
{{< var keybind.targets_make >}} to update the pipeline. If it all
...
install the helper package `{tarchetypes}` first:
...
{{< var keybind.targets_make >}}, the Quarto file also gets re-built.
But when we use `targets::tar_visnetwork()` using
{{< var keybind.targets_vis >}}, we don't see the connections with plot
...
Re-run `targets::tar_visnetwork()` using {{< var keybind.targets_vis >}}
to see that it now detects the connections between the pipeline targets.
Then, run `targets::tar_make()` with {{< var keybind.targets_make >}}
