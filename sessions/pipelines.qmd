...
```{r purl-only-renv-install-targets}
#| eval: false
#| echo: false
#| purl: true
...
```{r purl-only-renv-install-tidyverse-dplyr}
#| eval: false
#| echo: false
#| purl: true
...
Git history. Open up the `doc/lesson.qmd` file and create a new header
...
c(
  ...
  paste0(collapse = "\n") |>
  write_to_file("doc/lesson.qmd")
git_ci("doc/lesson.qmd", "Add section on basic stats to rmd")
```
...
While inside the `doc/lesson.qmd` file, use the "function-oriented"
...
the `list()`. Since we're using the `doc/lesson.qmd` as a sandbox, we
...
git_ci("doc/report.Rmd", "Add code for basic stats to report.")
```
...
styler::style_file("doc/report.Rmd")
git_ci("doc/report.Rmd", "Basic stats as a pretty table.")
```
