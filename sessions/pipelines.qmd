...
figures. Let's write some code to create the plots for our items 2 and 3
...
each metabolite. We use `scales = "free"` because each metabolite
doesn't have the same range of values (some are small, others are quite
large).
...
8.  Save both files and then open the Git interface and commit the
    changes you made to them.
9.  Repeat these tasks for the code for the metabolite distribution
...
## Adding the plots as pipeline targets

Now, let's add the two functions to the `_targets.R` file. Let's write
these two `tar_target()` items within the `list()` inside `_targets.R`.
To make it easier to track things, add `fig_` to the start of the `name`
given.
...
```{r purl-only-renv-install-tarchetypes}
#| eval: false
#| echo: false
#| purl: true
