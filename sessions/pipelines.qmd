...
it to our pipeline later. First, open up the `docs/learning.qmd` file and
...
Next, we'll switch back to `docs/learning.qmd` and write the code to this
...
1.  Wrap the plot code inside `docs/learning.qmd` with `function() {...}`
...
`docs/` folder and since `{targets}` by default looks in the current
...
Now, going back to the `docs/learning.qmd` file, by using
