...
analysis. So what is your **exact** workflow when you
...
This isn't to diminish the work of researchers, but is rather a basic observation
...
2.  Use `summarise()` to calculate the `mean()` and standard deviation (`sd()`) for each
    metabolite.
3.  Round the calculated mean and standard deviation to 1 digit
    using `round()`.
