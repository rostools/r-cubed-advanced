...
```{r purl-only-remove-use-data-data-raw}
#| eval: false
#| echo: false
#| purl: true
revise_by_text(
...
git_ci("data-raw/nmr-omics.R", "Use csv for data instead.")
```
...
```{r purl-only-ignore-data-csv}
#| eval: false
#| echo: false
#| purl: true
git_ci(".gitignore", "Don't track data.")
```
...
To get a bit familiar with how `{targets}` works, go into the
`_targets/` folder and take a look around. There are two important ones:
`meta/` and `objects/`. Look in each to get familiar with the files
there.
...
-   The `objects/` folder contains the output of each `tar_target()`
    (e.g. the data from `read_csv()` or the statistics from
    `descriptive_stats()`).
