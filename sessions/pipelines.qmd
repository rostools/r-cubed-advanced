...
Before beginning, encourage learners to recall what they remember from
the previous session, either with something like Mentimeter or verbally.
Preferably with something like Mentimeter because it allows everyone to
participate, not just the ones who are more comfortable being vocal to
the whole group.
:::

When working on a data analyses, you have probably experienced (many)
times where you forget the order in which other code should run or which
parts need to be rerun to update results. Things can get confusing
quickly, even in relatively simple projects. This becomes even more
challenging when you return to a project after a month or two and have
forgotten the state of the analysis and the project as a whole.
...
adding these steps to a pipeline that tracks those them, you can make
things a lot easier for yourself and others. This session focuses on
using tools that create and manage these pipelines effectively.
...
analysis all over again. So what is your **exact** workflow when you
need to rerun code and update your results? Assume it's a change
...
3.  For the remaining time, we'll briefly share with everyone what has
    been thought and discussed.
...
actual pipelines, because the individual steps are not very clear and
not well described, and they often requiring a fair amount of manual
human attention and intervention. Particularly within computational
...
Why aren't these data "pipelines" in most research actual pipelines?
Because:
...
as researchers are not necessarily trained in writing code, nor do we
have a strong culture and incentive structure around learning, sharing,
reviewing, and improving code. We are also very rarely allowed to get
(or use) funds to hire people who *are* trained and skilled in
programmatic thinking and coding Otherwise courses like this wouldn't
need to exist :woman_shrugging:
...
running on a Linux server) that we won’t cover in this course.
:::
...
get you started. But, notice `tar_target()` function used at the end of
the script. There are two main arguments for it: `name` and `command`.
The way that `{targets}` works is similar to how you'd assign the output
of a function to an object, so:
...
course. There are so many advantages to this type of workflow which is
why many powerful R packages are designed around making use of this type
workflow.
...
numbers to one digit. To do this, will utilize the split-apply-combine
methodology and the `across()` function, we covered in the
[functionals](https://r-cubed-intermediate.rostools.org/sessions/functionals)
session of the intermediate course.

We will:

1.  Use `group_by()` to split the dataset based on the `metabolites`.

2.  Use `summarise()` to calculate the `mean()` and `sd()` for each
    metabolite.

3.  Round the calculated mean and standard deviation (`sd`) to 1 digit
    using `round()`.

While doing this, we will also utilize the `across()` functional, making
...
commir the changes to the Git history with {{< var keybind.git >}}.
...
course](https://r-cubed-intermediate.rostools.org/sessions/functions#the-basics-of-a-function),
...
3.  At the start of each function we use inside our `descriptive_stats`
    ...
    E.g., add `dplyr::` before `group_by()` because it's from from
...
In the intermediate course, we suggested using `return()` at the end of
...
be using this feature throughout the rest of this course.
:::
...
actual data file we need to create a seperate pipeline target, where we
