...
project. Since packages can be used multiple times, it can also list the
packages more than once. To show only the unique packages used, we'll
use `unique()` and `$Package` (the variable name), as well as stopping
the function from outputting messages with `quiet = TRUE`:
