...
::: callout-important
This session has no coding. There is only reading activities, where the
instructor will explain again in different ways, along with discussion
activities.
:::
...
For any given value of $x$, the function $f(x)$ will give the expected
value of $y$. For example, you might expect a relationship between plant
growth, water, and sunlight. You could model plant growth as depending
on water and sunlight. Graphically, this model can be illustrated as:
...
perfect. First thing that a regression model has is a mathematical
...
relationship). What the research question and outcome are will determine
what type of regression model the formula is (e.g. a linear or logistic
regression).
...
as machine learning models like large language models to things as
simple as t-tests or ANOVAs.
...
use for the rest of the workshop. Emphasise that we will be using this
example and the information we've brainstormed in this activity to help
inform us how we will create the models in the next sessions.
...
A mathematical representation of this theoretical model could be (using
the variable names from the data):
