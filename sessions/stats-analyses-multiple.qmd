...
a header and code chunk at the end of the `doc/lesson.qmd` file:
...
this. Convert it into a function, add the `r run_roxygen_comments`,
style using {{< var keybind.styler >}}, move into the `R/functions.R`
...
`r run_roxygen_comments`, use {{< var keybind.styler >}} to style, move
...
3.  Use {{< var keybind.styler >}} to style and than run
    `r run_tar_vis_text` to see if the new target gets detected. If it
    does, than run `r run_tar_make_text`.
...
2.  Move the function into the `R/functions.R` file, add
    `r run_roxygen_comments`, and use {{< var keybind.styler >}} to
    style.
...
Create another code chunk at the bottom of `doc/lesson.qmd` to add this
code:
