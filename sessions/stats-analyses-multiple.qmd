...
In the `docs/learning.qmd`, use the new function in the code:
...
The last step is to `right_join()` to the estimates made from the
`calculate_estimates()`:
...
`docs/learning.qmd` file. Open it up and create another code chunk at
the bottom of the file. Like we did with the other outputs (like the
