# Extend your analysis to run many models {#sec-stats-analyses-multiple}
...
Alright, let's style this file by using {{< var keybind.style >}} before
...
4.  Once you have the code working, style the file with
    {{< var keybind.style >}}. Commit your changes with
...
2.  Style the file with {{< var keybind.style >}}.

3.  Once you've updated the function, check the targets visualization
    with {{< var keybind.targets-viz >}} to see how the pipeline has
    been changed. Check out the outdated targets with
    {{< var keybind.targets-outdated >}}.

4.  Then, run the pipeline with {{< var keybind.targets-run >}} to get
...
6.  Check the targets pipeline with {{< var keybind.targets-viz >}}
    again to see what has changed. Then run the pipeline again with
    {{< var keybind.targets-run >}}.
