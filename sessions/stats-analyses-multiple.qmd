...
TODO: Maybe not do this exercise?
...
4.  Add the `r run_roxgyen_comments`.
5.  Cut and paste the function over into the `R/functions.R` file.
6.  Run `r run_style_text` and `r run_lintr_text` (try to fix any
    issues).
...
Run `r run_tar_viz_text` and then run `r run_tar_make_text`. Once
...
Run `r run_tar_viz_text`, then `r run_tar_make_text`. We now have the
