...
7.  Create an R Project in @sec-, along with the folder and file setup.
...
10. Run a check to see that everything is as expected. You'll later need
...
example with `pak::pak()`, you would "read" this as:
...
this method because we only want to use the `pak()` command from the
`{pak}` package and not have to load all the other functions as well. In
this course we will be using `::` often.
