...
tasks in this section as well as **completing the survey** at the end.
These tasks are designed to make it easier for everyone to start the
course with everything ready to go. For some of the tasks, you might not
understand why you need to do them, but you will likely understand why
...
about them are found as you work through the section.

1.  Install as updated a version of R, RStudio, Quarto, and Git as
    possible. For some people, depending on their institution, this task
    can take the longest amount of time because you have to contact your
    IT to install these packages.
...
4.  Run a check to see if everything works.
...
8.  Run a check to see that everything is as expected
    -   You'll later need to paste this output into the survey.
9.  Read the [Welcome](index.qmd) page.
10. Read the [syllabus](content/00-syllabus.qmd).
11. Read the [Code of Conduct](CODE_OF_CONDUCT.md).
12. **Complete the pre-course survey**. This survey is pretty quick,
    maybe \~10 minutes.
...
## Notes on reading the course website
...
-   Functions always end with `()`, for instance `mean()` or
    `read_csv()`.
-   R package names are always wrapped with `{}`, for instance `{dplyr}`
    or `{tidyverse}`.
-   Sometimes functions have their package name appended with `::` to
    ...
    install packages from GitHub using the `{remotes}` package by using
...
1.  Have the latest version of
    ...
    any version above `4.1.2`).
2.  Have the latest version of
    [RStudio](https://www.rstudio.com/products/rstudio/download/#download)
    installed (or at least any version above `v2021.09.0+351`).

There are a few other things to install:

1.  [Git](https://git-scm.com/downloads). We'll be using Git (building
    off of previous courses), so it needs to be installed.
2.  For some Windows users, you may need to install
    [Rtools](https://cran.r-project.org/bin/windows/Rtools/Rtools35.exe)
    in order for some R packages to be installed (which you'll do
    shortly). For some computers, installing Rtools can take some time.
    You need this in order for some packages to work.
3.  [Quarto](https://quarto.org/docs/get-started/). Quarto is an
    upgraded version of R Markdown. If you've used R Markdown before,
...
it's popularity and the amount of documentation available for it. Check
...
2.  Install the `{r3}` helper package for this course and run the
    `install_packages_advanced()` function from it:

    ``` r
    remotes::install_gitlab("rostools/r3", upgrade = TRUE, dependencies = TRUE)
...
have used or currently use Git, you can skip this section. If you *have
not* used it, please do these tasks:

1.  Follow the [pre-course tasks for
    Git](https://r-cubed.rostools.org/pre-course.html#setting-up-git-and-github)
...
2.  Or, running the command
    `prodigenr::setup_project("~/Desktop/AdvancedR3")` in the R Console.
...
Here we use the [`{usethis}`](https://usethis.r-lib.org/) package to
help set things up. `{usethis}` is an extremely useful package for
managing R Projects and we highly recommend checking it out more to see
how you can use it more in your own work.
...
Quarto is a upgraded version of R Markdown and chances are, if you use a
fairly recent version of RStudio, you are already using it without
realizing it. That's because Quarto uses the same Markdown syntax as R
Markdown. The only difference is that with Quarto, you can create more
types of output documents (like books, websites, slides), you have more
options for customization, and it's easier to do and learn than R
Markdown. So, for this course, we will eventually use Quarto to make a
website of the analysis that we will do over the course.
...
2.  Open up the `AdvancedR3` project, either by clicking the
    `AdvancedR3.Rproj` file or by using the "File -\> Open Project"
    menu. Run the function below in the Console when RStudio is in the
    `AdvancedR3` project, which will create a new file called
    `lesson.Rmd` in the `doc/` folder.
...
intermediate course where I wrote through each step of the script and
what we needed to do, in this course I am only providing the script to
download and minimally process the dataset into an initially workable
state. If you want to learn more about how to take a raw data set and
process it into a format that is more suitable for analysis, check out
the [intermediate course](https://r-cubed-intermediate.rostools.org/).

Inside the `data-raw/` folder, we are going to write R code that
downloads a dataset, processes into a more tidy format, and save it into
the `data/` folder. This is the start of our analysis pipeline. First
step, we need to create the script to do these steps. While in your
`AdvancedR3` R Project, go to the Console pane in RStudio and type out:

```{r create-data-raw, eval=FALSE, purl=TRUE}
...
lipidomics `.xlsx` file. For now, we only want the lipidomics dataset.
...
```{r insert-data-raw-script, file=here::here('data-raw/nmr-omics.R'), eval=FALSE}
```
...
the end, though, the only important thing in the end is to run them all
and get the `lipidomics` dataset to be saved as `data/lipidomics.rda`.
The created files should look like this:

```{r file-tree-data-folders, echo=FALSE}
fs::dir_tree(here::here("."), glob = "data|data-raw")
```
...
At a look through the downloaded `data-raw/nmr-omics/README.txt` and
`data-raw/nmr-omics/lipidomics.xlsx` files, as well as the created
`data/lipidomics.rda` to start better understanding the dataset.
...
```{r ignore-xlsx-files, eval=FALSE, purl=TRUE}
usethis::use_git_ignore("data-raw/nmr-omics/")
```

TODO: Script to check project?
...
doesn't, start over by deleting all *but* the `data-raw/nmr-omics.R`
script and re-running the script again. If your output looks a bit like
the above, than copy and paste the output into the survey question at
the end.

## Course introduction

Most of the course description is found in the
[syllabus](content/00-syllabus.qmd). If you haven't read it, **please
read it now**. Read over what the course will cover, what we expect you
to learn at the end of it, and what our basic assumptions are about who
you are and what you know. The final pre-course task is a survey that
asks if you've read it and if it matches you.
...
We have a [Code of Conduct](appendix/conduct.qmd). If you haven't read
it, **read it now**. The survey at the end will ask about Conduct. We
want to make sure this course is a supportive and safe environment for
learning, so this Code of Conduct is important.

You're almost done. Please fill out the [**pre-course survey**]() to
