...
for the pre-course tasks and finish with **completing the survey** at
...
7.  Write R code to download the data and save it to your computer. This
    task will probably take up the most time, maybe 30-60 minutes.
8.  Run a check to see that everything is as expected
    -   You'll later need to paste this output into the survey.
9.  Read the [Welcome](index.qmd) page.
10. Read the [syllabus](content/syllabus.qmd).
...
Given this is an intermediate course, you should already have R and
RStudio installed. However, you may not have the latest versions
installed.

1.  Have the latest version of
    [R](https://cloud.r-project.org/index.html) installed (any version
    above 4.2.0).
2.  Have the latest version of
    [RStudio](https://www.rstudio.com/products/rstudio/download/#download)
    installed (any version above 2022.02.3+492 TODO: Update this?).

There are a few other things to install:

1.  [Git](https://git-scm.com/downloads). We'll be using Git (building
    off of the Introduction course), so it needs to be installed.
...
> R or RStudio, if you have at least 4.2.0 for R and at least
> 2022.02.3+492 TODO: Update this? for RStudio, that should be fine. If
> you have versions of R and RStudio older than that, it is necessary
> for you to request that IT update your software if you can't yourself.
> Unfortunately, given that Git is not a commonly used software for some
> organizations, you may not have it installed and you *will* need to
> ask IT to install it. We *require* it for the course, so please make
> sure to give IT enough time to be able to install it for you.

Once R, RStudio, and Git have been installed, open RStudio. *If at any
point during these pre-course tasks you have any troubles*, try as best
...
try to book a quick video call to fix the problem. Otherwise, if you
can, come to the course earlier (about 20-30 min) to get help.
...
packages](https://r-cubed.rostools.org/pre-course.html#installing-r-packages)
...
`install_gitlab()` command from the remotes package and not have to load
...
the [session](https://r-cubed.rostools.org/version-control.html) on it
...
7.1](https://r-cubed.rostools.org/r-project-management.html#what-is-a-project-and-why-use-it)
...
We teach and use R Markdown because it is one of the very first steps to
...
2.  Open up the `LearnR3` project, either by clicking the
    ...
    `LearnR3` project, which will create a new file called `lesson.Rmd`
    in the `doc/` folder.
