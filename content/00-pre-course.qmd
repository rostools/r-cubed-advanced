...
well as for general setup, so we have defined the following learning o
for this page:
...
2.  Install the `{r3}` helper package for this course and run the
    `r3::install_packages_advanced()` function from it:
...
2.  Or, running the command
    ...
    location like `Documents`) in the R Console.

```{r run-prodigenr, echo=FALSE, eval=FALSE, purl=TRUE}
# For purling
...
```{r more-project-setup, eval=FALSE, purl=TRUE}
...
2.  Open up the `AdvancedR3` project, either by clicking the
    ...
    ```{r add-rmarkdown-doc, eval=FALSE, purl=TRUE}
...
```{r file-tree-data-folders, echo=FALSE, comment=""}
fs::dir_tree(glob = "data|data-raw")
```
