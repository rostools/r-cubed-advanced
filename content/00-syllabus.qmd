...
of, and needed by, scientists and researchers in modern research
environments. Work environments and projects require a high level of
hands-on collaboration throughout all stages of the research life cycle.
We are faced with obstacles that we have no training for nor knowledge
on how to address. Obstacles can include not having a shared way of
writing code or managing files, which can impede effective
collaboration. Other obstacles includes the demand for documenting
software dependencies or steps in an analysis pipeline in a way that
makes it easy to resolve and get the most recent set of results after
collaborators have worked on a project. Even aside from the impact on
collaboration, these barriers can also affect projects with one primary
researcher. If not resolved, this can affect the reliability and
reproducibility of scientific results, especially considering that if
these barriers are addressed, and obstacles are overcome, the results
are often not explicitly shared in classical scientific outputs like
publications.
