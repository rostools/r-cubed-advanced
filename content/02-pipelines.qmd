...
-   Building reproducible analysis pipelines: targets
    ...
    -   Add targets to repo
    -   list out, what are the things we want to create, then go and
        create them. (will do this for next session too.)

Image of steps in a pipeline, what intermediate items there are, etc.
...
gert::git_add("R/functions.R")
gert::git_commit("Add function to create descriptive stats.")
```
...
" |>
  readr::write_lines(here::here("R/functions.R"), append = TRUE)
gert::git_add("R/functions.R")
gert::git_commit("Add function to switch output for targets use.")
```
...
" |>
  readr::write_lines(here::here("R/functions.R"))
gert::git_add("R/functions.R")
gert::git_commit("Add function to create plot of count stats.")
```
