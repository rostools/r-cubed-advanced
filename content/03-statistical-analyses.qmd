...
list item. Like we did with the `metabolite_to_wider()`, we need to pipe
the output into another `map()` function that has a custom function that
runs the models. We don't have this function yet, so we need to create
it. Convert the code we've used previously into a function, replacing
`lipidomics` with `data` and using `starts_with("metabolite_")` within
the `create_recipe_spec()`. Add the `r run_roxygen_comments` and move
...
```{r exec-only-chain-generate-model-results}
#| eval: true
#| echo: false
.Last.value %>% 
  map_dfr(generate_model_results)
...
```{r exec-only-chain-filter-terms}
#| eval: true
#| echo: false
.Last.value %>% 
  filter(str_detect(term, "metabolite_"))
...
Wow! We're basically at our first `{targets}` output! This is a good
place to convert this code into a function and add a `{targets}`
pipeline for it.

## Exercise: Adding model results as a target in the pipeline

> Time: \~15 minutes.

```{r}
calculate_estimate <- function(data) {
  data %>%
    column_values_to_snakecase(metabolite) %>%
...
model_estimates <- lipidomics %>% 
  calculate_estimate()
# targets::tar_read("model_estimates") 
```
...
```{r}
set.seed(1324)
bootstraps(lipidomics_wide, times = 10)

set.seed(1324)
bootstrap_samples <- create_model_workflow(
  logistic_reg() %>% 
    set_engine("glm"),
  lipidomics_wide
)
