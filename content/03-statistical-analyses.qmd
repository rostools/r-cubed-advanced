...
regression model and the results need to be corrected to be more
interpretable:
...
```{r}
%>% 
  map_dfr(~fit_model_to_tidy_output(.x, starts_with("metabolite")), .id = "model_id")
all_metabolites_results %>% 
  filter(str_detect(term, "metabolite_"))
```

```{r}
fit_model_to_tidy_output <- function(data, metabolite) {
  create_model_workflow(
    logistic_reg() %>%
      set_engine("glm"),
    data %>%
      create_recipe_spec({{metabolite}})
  ) %>%
    fit(data) %>%
    tidy_model_output()
}
...
``` r
set.seed(1324)
bootstraps(df_lipid_ch_3_1, times = 10)
