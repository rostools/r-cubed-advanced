# Setup and connect with GitHub {#sec-connect-github}
...
But first, create an account on GitHub if you don't already have one by going to
[github.com/join](https://github.com/join). Next, we need to connect (or rather, authenticate)
...
always check it in R. You can check by using the `{usethis}` package, which
you need to install first. Open up RStudio and in the Console, type:

``` {.r filename="Console"}
install.packages("usethis")
```

Once that is installed, you can check if you have a PAT already set up by typing:
